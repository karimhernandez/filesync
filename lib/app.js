...
function init() {
    ...
    try {
        var configPath;
        if (!argv.config) {
            if (fileExists(process.cwd() + '/app.config.json')) {
                configPath = process.cwd() + '/app.config.json';
            } else {
                console.log('No config found.  Use the config argument (eg. --config app.config.json)'.red);
...
function watchFolders() {
    ...
    chokiWatcher = chokidar.watch(watchedFolders, {
            ...
            // ensures stats always available (KH)
            alwaysStat: true
        })
        .on('add', function (file, stats) {
